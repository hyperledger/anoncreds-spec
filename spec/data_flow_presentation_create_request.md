...
The Verifier starts the process in step 1 of the [AnonCreds Presentation Data
Flow](#anoncreds-presentation-data-flow) by creating and sending a presentation
request to the Holder.

The presentation request provides information about the attributes and
predicates the verifier is asking the the Holder to reveal, restrictions on what
verifiable credentials can be the sources for the attributes and predicates, and
limitations on the freshness of the credential revocation status. Presentation
requests are at the "business logic" layer, and do not have any cryptographic
component. The verification process includes verifications that the presentation
satisfies the request. The verifier SHOULD validate that the presentation
satisfies the business requirements for which the presentation was provided.
...
* `nonce` is a string, a decimal, 80-bit number generated by the verifier that
  SHOULD be unique per presentation request. The nonce is used
  to prevent replay attacks is used in creating and verifying the presentation.
...
* `restrictions` is a condition on the source credential that can be used to
  ...
  * Omitting `restrictions` implies that:
    * The holder MAY provide self-attested attributes for the request in the
    presentation, or
...
* `name` (required) is a string, the name of the attribute from a source
  ...
  * To be useful, the attribute in the source credential MUST be an integer, but
    that cannot be enforced. The verifier MUST understand how the attribute is
...
"((issuer_did = <did> or schema_id = <id>) AND (cred_def_id = <id>") and (the
credential must contain an attribute "color" with the value "red"))"`

#### Request Non-Revocation Proofs

The presentation request JSON item `non_revoked` allows the [[ref:
...
As noted in the presentation request specification above, a `non-revoked` item
...
[[ref: VDR]] to get additional [[ref: RevRegEntry]] data. A verifier using a
`from` of `0` (start of time) allows the [[ref: holder]] to respond with a
non-revocation proof at any point in time, including before it was revoked.

In practice, the use of the interval is not well understood and tends to cause
confusion amongst those building presentation requests. The AnonCreds community
recommends using matching `from` and `to` values as outlined in the [Aries RFC
0441 Present Proof Best
...
The verifier can then decide if the revocation is sufficiently up to date.

While one might expect the `to` value to be the current time ("Prove the
...
The following is an example of a full presentation request for a presentation
...
the [[ref: verifier]] sends the presentation request to the [[ref: holder]].
